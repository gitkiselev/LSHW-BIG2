...
function init() {
    var myMap;
    var reviewFormAddress = document.querySelector('.header__title');//Адрес места отзыва
    var addReview         = document.querySelector('.feedback__link');
    var areaUser          = document.querySelector('.feedback-area__user');

    var areaReview        = document.querySelector('.feedback-area__review');

    var btnClose          = document.querySelector('.header__close-btn');//Закрыть попап окно
    //Переменные формы
    var userName          = document.querySelector('.feedback__name');
    var userPlace         = document.querySelector('.feedback__place');
    var userMessage       = document.querySelector('.feedback__text');

    var addReview         = document.querySelector('.feedback__link'); //Добавить  

    var reviewForm        = document.querySelector('.app');//Окно
...
//Кластеризатор
    ...
    function popupShow(e) {
        ...
        document.querySelector('.feedback-area__list').innerHTML = "Отзывов пока нет";
        coords = coord;
        coordX = coords[0];
        coordY = coords[1];
    ...
    function renderReviews(address) {
        ...
        xhr.onreadystatechange = function () {  // Формируем функцию срабатывания на успешный ответ от сервера
            if (xhr.readyState === 4) {
                ...
                var content = document.querySelector('.feedback-area__list');
                var source = reviewTemplate.innerHTML;
